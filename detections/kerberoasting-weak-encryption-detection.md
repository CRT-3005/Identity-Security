...
In a hardened domain enforcing **AES-only Kerberos encryption**, any non-AES service ticket is treated as a **high-confidence regression indicator** that may enable offline cracking of service account tickets.

---

## 🔍 Detection Background

Kerberoasting is a credential access technique where an attacker requests Kerberos service tickets for service accounts and cracks them offline. The feasibility of this attack increases when service tickets are issued using weaker encryption types such as RC4.

This detection is designed as both a **visibility control** and a **regression control**:

- Validate that the environment is issuing **AES256 service tickets (`0x12`)**
- Filter out normal Kerberos noise such as computer accounts and `krbtgt`
- Alert only when a ticket is issued using **non-AES encryption**, excluding known logging edge cases such as `0xffffffff`

---

## 🔍 Detection Workflow

This detection was developed using a structured validation approach:

1. Baseline Kerberos service ticket encryption across the environment
2. Narrow the dataset to Kerberoast-relevant service accounts
3. Detect non-AES service tickets as potential regression or exposure indicators
...
Kerberoasting targets **user-based service accounts**, not computer accounts. Computer accounts typically end with `$`, and `krbtgt` is excluded because it is not a roastable service target.
...
The final detection alerts when Kerberos service tickets are issued using **non-AES encryption** in Kerberoast-relevant contexts.

Notes:

- `0x12` represents AES256 tickets and is expected in this environment
- `0xffffffff` can appear in Kerberos telemetry as an edge-case value where the encryption type is not explicitly logged, so it is excluded to reduce noise
...
In an AES-hardened domain, this acts as an effective regression control that helps identify configuration drift or suspicious ticket issuance that could increase exposure to offline cracking of service account tickets.
